...
1. 只读记录新官方包版本、管道归属、工具目录与实际工具调用封套，比较名称、schema、owner 发现和实时事件。不得重启或改写官方安装、历史或账号凭据。
   可先运行 node scripts/verify-compatibility-live.mjs：读取现有官方管道和工具目录，再只读调用一次 `list_projects`；只输出版本、方法是否存在、字段是否缺失与调用状态，不记录项目、任务内容或凭据。工具调用成功不等于新版写入通过。

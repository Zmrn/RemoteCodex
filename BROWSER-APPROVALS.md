...
2026-09-12：另行接入终端命令审批，见 COMMAND-APPROVALS.md。共用卡片、POST及防重复提交逻辑，使用不同官方接口与能力判断；旧网站字段不混入终端命令，新命令功能尚未打包。

2026-09-10 源码功能已随 0.10.31 在 GitHub 构建并正式发布，见 GITHUB-UPDATES.md。Windows 和 Android 共用会话内授权卡，目标电脑也需要更新。未安装或执行 APK；真实授权往返仍未验证。

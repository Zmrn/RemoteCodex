...
会话首屏按最多 40 项 / 约 128 KiB 消息体分段读取（单个超长消息仍保留完整内容）。向上滚动或点击“加载更早的消息”继续读取；刷新不会丢掉已加载历史。图片接近可见区域时单独下载。分页查看缓存有数量、内存和时间上限，过期或服务重启后需重新打开会话，官方历史不会被改写。双方都更新到 0.9.8 才能获得完整减量效果；连接旧服务仍可读，但旧端继续发送原来的较大响应。
...
# 发布到用户授权的 tx 资源服务
python scripts/publish-update.py dist/RemoteCodex.exe --version 0.9.8
# 只读诊断

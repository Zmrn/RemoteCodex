# Windows 单 EXE 桌面版 0.10.2

2026-09-18：0.10.42已发布程序缓存落盘和自动恢复：解压逐文件刷盘、完整校验后发布，损坏时从内置资源生成独立恢复缓存，保留原目录及所有用户数据。中途退出和并发启动有保护。0.10.41及更早版本尚未包含此改动，需更新到0.10.42；说明与复测见 [CACHE-RECOVERY.md](CACHE-RECOVERY.md)。

窗口大小、位置及最大化状态保存在 `%LOCALAPPDATA%/RemoteCodex/data/window-placement.json`（指定 `--home` 时位于该目录的 `data`）。调整后自动保存，隐藏到托盘、真正退出及升级前立即保存；重新打开恢复。首次启动默认占屏幕可用区域约 90%，最大 1440×960。更换到较小屏幕或断开副屏时，窗口会缩小并移回当前可用区域，不遮到任务栏外。最小化不会将图标位置或最小化尺寸写入配置。

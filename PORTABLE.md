...
0.9.9 会在断线后自动重试，最长重试间隔 30 秒，事件通道无数据 45 秒后重新连接。首次连接失败也继续尝试；恢复时重新订阅并读取同一任务，保留草稿、图片和历史。仅重试查看连接，不重发消息、队列、审批或设置。官方应用需要保持运行；关闭 Remote Codex 后停止重试。
...
# 发布到用户授权的 tx 资源服务
python scripts/publish-update.py dist/RemoteCodex.exe --version 0.9.9
# 只读诊断

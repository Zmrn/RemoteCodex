...
0.9.3 的重复启动保护覆盖不同 EXE 副本、版本和 `--home` 数据目录：同一 Windows 用户只允许一个桌面实例，后续启动聚焦已有窗口。诊断和更新预检是短暂进程，不创建另一套桌面服务。
...
# 发布到用户授权的 tx 资源服务
python scripts/publish-update.py dist/RemoteCodex.exe --version 0.9.3
# 只读诊断

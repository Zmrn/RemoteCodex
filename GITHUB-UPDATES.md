...
最终源码 `86df8c1870c237235219c8a4473f8bed09b49567` 的 Checks `36170576480` 成功；同 SHA preflight 后仅派发一次 Build `36171158095`（requestId `ef36ed2f-3822-4687-be39-23f7c71d553f`），云构建成功。发布上传遇两次网络中断：首次草稿有 1 项，第二次有 6 项；均核对草稿后沿该 Build 继续，未重新构建或覆盖公开资源。14 项资源逐项读回后公开；匿名 latest 四份签名清单、固定版本四包及说明完整下载并与构建哈希一致。

| 安装文件 | 字节数 | SHA-256 |
| --- | ---: | --- |
| RemoteCodex.exe | 44203520 | `5fd7a4db0176e1f2de33eb74b46edb7c45392dc0d6ad8c13cdc5d3ca0a09993a` |
| RemoteCodex.apk | 295152 | `0133e52a4a1f2b5803a7211e11a6b12383a4267d76355ebc85103548dbeebc57` |
| LimitRemoteCodex.exe | 44203520 | `01af82bbbd511e5d77fd76057598d000d6aef4a0f0263210113d54a41e2e3e1e` |
| LimitRemoteCodex.apk | 295152 | `d49006121e0202dd8a751a2097158e00ba69e395b20b9380b3c86d33bcc39518` |

最终版本 363 项 Node22.19.0、桌面／手机发送恢复界面、兼容清单、两版各 27 份 Android Java 源码 API35 编译通过。四份原 RSA 更新清单与包哈希一致；两个 APK 保持原证书 SHA-256 `3c0a98ec3c9f37318525f5d0e4afb3417812215d625e48a9013b5ee649acb2b1`、v2/v3 签名、独立包名和 versionCode `10050`。两个 EXE 解包源码、共享界面、更新渠道及发布说明与本提交一致，内嵌 Node22.19.0/Python3.13.2、DPAPI 与页面自检通过。隔离旧云包 0.10.49→0.10.50 的两版设备 ID、加密密钥和配置字节保持，更新后连续两次读取通过。

完整版 EXE 在本机官方 Windows x64 `26.917.6896.0` 上只读连接及项目／会话列表成功；Limit EXE 自检确认不访问本机官方。该只读核对不等于真实发送行为验证；本次没有向用户任务发送测试消息，未执行 APK／模拟器或升级现有客户端。普通 Chat 新建／图片及未完成 Work 未纳入；主工作区八份未完成 Chat 修改未进入构建。Windows EXE 仍无 Authenticode 签名。证据在 `work/release-050/work/`；本记录提交不再次打包，四包仍对应上述构建源码。
...
Limit 版与完整版独立安装、保存设备和更新状态。目标完整版通过受限配对码隔离会话：同一码共享自己创建的会话，不同配对身份相互不可见；受限客户端不能连接本机官方应用，不能查看或选择项目，只能创建无项目会话，不额外收紧会话内工具权限。目标端可更换或撤销配对码。额度历史的一天视图增加本地日期分割线。使用 Limit 版前应先更新目标电脑的完整版 Windows 接入端；已发布旧包不会自动变成 Limit 版。

| 产物 | 字节数 | SHA-256 |
| --- | ---: | --- |
| RemoteCodex.exe | 44202496 | `2abab2be2fca5d1ca5d5ee4eeca37b2eb8fea092d1d53a90044fadac4e1baa0b` |
| RemoteCodex.apk | 295152 | `144b9a2c3289112bc61739a777d7dadb8d52173dbac17037d25d4cdb6f233a07` |
| LimitRemoteCodex.exe | 44202496 | `8676dac59d2c48a773b819e0f3b64607031814fea9398c07ecdbfda9264f9238` |
| LimitRemoteCodex.apk | 295152 | `3ccb182b58fc03739148dd26cc6e2e1b3f6d9f20013a83f59777f6c7e10a374e` |

四包均通过原 RSA 更新清单验签、同提交/版本/兼容清单与源码比对；两个 APK 使用原证书 SHA-256 `3c0a98ec3c9f37318525f5d0e4afb3417812215d625e48a9013b5ee649acb2b1` 和 v2/v3 签名，包名分别为 `com.anso.remotecodex` / `com.anso.limitremotecodex`，versionCode 均为 `10047`。中央兼容清单 SHA-256 `974400d14d15b89af96d6a8826a9e39f3b42a69d1482541532e46e4897ca3158`。两版 EXE 内置 Node 22.19.0/Python 3.13.2、DPAPI 与页面资源隔离自检通过；Limit 包自检明确不发现或连接本机官方应用。包内各 110 份生产源码/资源及 APK 页面与本次发布提交一致，未包含用户配置、私钥或未提交 Chat 工作。

最终版本 360 项 Node 22.19.0、Limit 界面与额度历史双尺寸回归、25 项 Android 更新网络主机 JVM 检查、两版各 27 份 Android Java 源码 API 35 编译和设备连接主机检查通过。隔离旧云包 0.10.46→0.10.47 的完整版设备 ID/加密密钥保存，以及 Limit 版独立数据重启读取通过。完整版包在本机官方 Windows x64 `26.917.6896.0` 只读连接、项目/会话列表成功；这不等于该新版全部功能行为验证。历史核心行为验证版本仍为 `26.901.6511.0` / `26.903.8094.0`，其他版本按接口逐功能判断。Chat 保留既有已完成能力，普通 Chat 新建/图片及未完成 Work 未纳入；主工作区八份未完成 Chat 修改未打包。

发布后匿名 `latest` 的四份签名清单、固定版本完整四包及说明文件均下载并与本次包哈希一致。未执行 APK/模拟器、升级现有客户端、重启官方应用、创建真实受限会话或向既有任务发测试消息；受限配对跨身份行为在隔离 HTTP 测试中验证。Windows EXE 未新增 Authenticode 签名。发布记录提交不再次构建，安装包固定对应上述源码 SHA。

## 2026-09-22：0.10.46 同路径图片缓存修复正式发布

[正式版本](https://github.com/Zmrn/RemoteCodex/releases/tag/v0.10.46)。0.10.46从958701c5840cf489fffafcad730d920dcaf3eabd经同SHA Checks35709621637和preflight成功后，由Build35710116905同次生成APK/EXE并正式发布Releases/v0.10.46，包含同路径图片覆盖后的版本识别、官方消息身份隔离及缓存刷新。仅派发一次，本次最终版本Checks与云构建首次成功。358 Node22.19.0、覆盖双尺寸10组及原图片稳定/Chat回归通过；原RSA/APK v2-v3证书、双端源码/界面/清单/渠道、EXE隔离运行时/DPAPI/SQLite、0.10.45→0.10.46设备保存/强制重启/删除和匿名完整下载通过。包内官方Windows x64 26.915.4065.0只读连接/列表、21/21接口及25/25功能依赖匹配；包内真实隔离图片/HTTP/共享界面覆盖双尺寸10组与额度重启读取通过。未执行APK/模拟器、升级现有客户端、重启官方或写用户任务；未完成Chat/Work未纳入。完整结果见GITHUB-UPDATES.md。

requestId `d5fa0911-c9d3-429e-a359-1f636c973fd2`；一次派发，构建首次成功。修复Agent覆盖图片文件后再次发送同一路径预览，Remote仍复用旧下载的问题。目标端使用文件变化和官方轮次/消息身份生成版本提示；新消息即使文件元数据碰巧相同也重新读取。前端下载缓存、节点和断点隔离采用一致的版本，迟到旧下载不会覆盖新版，未变图片正常刷新仍保持已解码内容。

请同时更新目标Windows接入端和Windows/Android控制端，无需重新配对。点击新预览放大或下载会取得新版；已经打开的放大窗口保留点击时快照，重新打开查看当前预览。第三方HTTPS直链图片不在此次范围。

| 产物 | 字节数 | SHA-256 |
| --- | ---: | --- |
| RemoteCodex.exe | 44195328 | `11814c3835c2f028940bf4a0de35af93015697862bf9fa30be1aa78c1516b85e` |
| RemoteCodex.apk | 291056 | `f44acd7ce7137e1c83145a4d661d8afabbc321fb32f2be9336d458d93947e7ac` |

原RSA更新签名、APK v2/v3原证书、包名com.anso.remotecodex/versionCode10046、双端源码/共享页面/说明/兼容清单/GitHub渠道一致，无用户配置或私钥。APK证书SHA256 `3c0a98ec3c9f37318525f5d0e4afb3417812215d625e48a9013b5ee649acb2b1`；中央清单 `974400d14d15b89af96d6a8826a9e39f3b42a69d1482541532e46e4897ca3158`。此处不是新增Windows Authenticode签名。

实际云EXE独立home、仅System32 PATH下内嵌Node/Python与DPAPI自检通过。包内当前官方只读列表47项、约232毫秒。包内Node22.19.0、生产模块/共享页面、真实隔离文件与HTTP在桌面/手机尺寸共10组核对覆盖、放大/下载、全部元数据碰撞的新消息、迟到旧请求、原子替换及删除重建。包内SQLite真实额度采样、新进程及官方断开后1/7/30天读取通过。

0.10.45→0.10.46两个真实云包源码的隔离HTTP/UI/DPAPI设备保存、强制结束测试子进程后保留、显式删除通过。当前客户端6项设备/接入/更新/通知配置和通知草稿字节保持。原8份Chat工作独立保全。

历史核心行为验证版本26.901.6511.0/26.903.8094.0（Windows x64）保持，其他版本按当前接口逐功能启用；本次接口匹配不等于全部功能行为重测。Codex保留现有已完成能力，Chat列表/阅读及受能力约束的文字续写保持，文字续写未在本轮真实重测；普通Chat新建/图片与未完成Work不在包中。未执行APK/模拟器、升级现有客户端、重启官方、写用户任务或复测断开RDP。

八项资源草稿上传、逐项读回后公开；匿名latest双签名清单、固定版本完整APK/EXE及说明核对通过。源码阶段首次Checks35707974820曾发现全部文件元数据碰撞，之后修正消息身份隔离及对应回归；最终源码Checks35708688352成功。本次最终版本Checks与Build均首次成功，不能将此前源码开发概括为无失败。证据work/release-046/work/。发布记录提交不再次构建，产物仍固定上述源码SHA。

## 2026-09-22：0.10.45 完整权限预设正式发布

[正式版本](https://github.com/Zmrn/RemoteCodex/releases/tag/v0.10.45)。0.10.45从45afa120ac99cb6750e4bda3c478b723ec48c466经同SHA Checks35705069728和preflight成功后，由Build35705515354同次生成APK/EXE并正式发布Releases/v0.10.45，包含完整官方权限预设与新建准备状态核对。仅派发一次，云构建首次成功。354 Node22.19.0、权限双尺寸6组及相关回归通过；原RSA/APK v2-v3证书、双端源码/界面/清单/渠道、EXE隔离运行时/DPAPI/SQLite、0.10.44→0.10.45设备保存/强制重启/删除和匿名完整下载通过。包内官方Windows x64 26.915.4065.0只读连接/列表、21/21接口及25/25功能依赖匹配；3个新专用Probe复现旧权限组合并通过修正后新建、Remote重连/下一轮、切回只读。官方旧任务恢复权限丢失仍有独立边界，不宣称彻底修复。未执行APK/模拟器、升级现有客户端、重启官方或写既有用户任务；未完成Chat/Work未纳入。完整结果见GITHUB-UPDATES.md。

requestId `9df5d3bb-c0b7-49f6-aa22-d58aacb97ee2`；一次派发，构建首次成功。修复“完全访问”只改范围而遗留旧审批策略、进入新任务显示“自定义权限”。内置选择现在同时提交官方配置ID、approvalPolicy和approvalsReviewer；新建准备核对当前连接/任务/owner及完整有效设置，确认后才派发。未知不重放，不私存旧权限覆盖官方状态。

请更新目标Windows接入端，建议控制端保持同版，无需重新配对。今天官方恢复老任务时未恢复旧设置而采用默认工作区权限，是另一条已确认的路径；没有重启官方验证新的配置是否能避免所有恢复问题，本版不对此作彻底修复承诺。

| 产物 | 字节数 | SHA-256 |
| --- | ---: | --- |
| RemoteCodex.exe | 44194816 | `d724deb984c85fd4050f6d07cdea88449195bdd1eb4f24c84eb0515a2b6890b5` |
| RemoteCodex.apk | 291056 | `d766debb4adb60e44a975f013b2b242fcc9ac68234a347253be0247e0363b755` |

原RSA更新签名、APK v2/v3原证书、包名com.anso.remotecodex/versionCode10045、双端源码/共享页面/说明/兼容清单/GitHub渠道一致，无用户配置或私钥。APK证书SHA256 `3c0a98ec3c9f37318525f5d0e4afb3417812215d625e48a9013b5ee649acb2b1`；中央清单 `974400d14d15b89af96d6a8826a9e39f3b42a69d1482541532e46e4897ca3158`。此处不是新增Windows Authenticode签名。

实际云EXE独立home、仅System32 PATH下内嵌Node/Python与DPAPI自检通过。包内当前官方只读列表44项、约2279毫秒。包内模块和Node22.19.0在3个新登记Probe中完成旧策略复现、纠正准备任务和新建首轮、Remote重连/后续轮次及切回只读；只发无工具READY指令。包内SQLite真实额度采样、新进程及官方断开后1/7/30天读取通过。

0.10.44→0.10.45两个真实云包源码的隔离HTTP/UI/DPAPI设备保存、强制结束测试子进程后保留、显式删除通过。当前客户端6项设备/接入/更新/通知配置和通知草稿字节保持。原8份Chat工作独立保全。

历史核心行为验证版本26.901.6511.0/26.903.8094.0（Windows x64）保持，其他版本按当前接口逐功能启用；本次接口匹配与权限验证不等于全部功能行为重测。Chat列表/阅读及受能力约束的文字续写保持，文字续写未在本轮真实重测；普通Chat新建/图片与未完成Work不在包中。未执行APK/模拟器、升级现有客户端、重启官方、写既有用户任务或复测断开RDP。

八项资源草稿上传、逐项读回后公开；匿名latest双签名清单、固定版本完整APK/EXE及说明核对通过。首次公开后Windows latest的302仍短暂指向0.10.44，固定0.10.45清单及双包均正确；随后原入口自然更新到0.10.45，原严格验签/哈希检查全部通过，未重建或重传。证据work/release-045/work/，保留首次失败与重查日志。发布记录提交不再次构建，产物仍固定上述源码SHA。

## 2026-09-21：0.10.44 图片续传与任务激活正式发布

[正式版本](https://github.com/Zmrn/RemoteCodex/releases/tag/v0.10.44)。0.10.44从b8f43f8ec490f4d721219607860e7608857c2209经同SHA Checks35523231653和preflight成功后，由Build35523481362同次生成APK/EXE并正式发布Releases/v0.10.44，包含图片/GIF断点续传与未加载任务激活。仅派发一次，云构建首次成功。346 Node22.19.0、图片进度/激活双尺寸各10组及相关回归通过；原签名/证书、双端源码/界面/清单/渠道、EXE隔离运行时/DPAPI/SQLite、0.10.43→0.10.44设备保存/强制重启/删除与匿名完整下载通过。包内官方Windows x64 26.915.4065.0只读连接/列表、21/21接口与25/25功能依赖匹配；专用Probe通过包内HTTP从notLoaded变resumed/idle，约2.34秒、导航1次、消息写入0。包内真实双HTTP转发图片续传6组与额度重启读取通过。未执行APK/模拟器、升级现有客户端、重启官方或写用户任务；Chat/Work未完成内容未纳入。完整结果见GITHUB-UPDATES.md。

requestId `63a9c303-900e-464a-8cec-a29baf23739a`；仅一次派发，构建首次成功。持续图片下载不再受75秒总时限中止，改为无数据闲置超时；有强ETag的断点重试请求剩余字节，原图变化则完整重下，最后SHA256校验。断点仅在页面内存，最多96MiB/30分钟，应用重启后需重下。

当前选中的未加载Codex任务通过官方已有导航指令加载，须取得同连接新鲜owner的resumed和明确运行状态；导航会切换目标官方当前显示任务。失败可重试、草稿可继续编辑、不发占位消息、不重放未知写入。请同时更新目标Windows接入端与控制端，无需重新配对。

| 产物 | 字节数 | SHA-256 |
| --- | ---: | --- |
| RemoteCodex.exe | 44194816 | `304d1a173f2b7ee215ce0f34c0443e73692bb35e8fefb84cfcbd396d535229cc` |
| RemoteCodex.apk | 291056 | `29063a10f7d8464731df5b36383cfd1f85f3502964d10d639c0c9ab114771194` |

原RSA更新签名、APK v2/v3原证书、包名com.anso.remotecodex/versionCode10044、双端源码/共享页面/说明/兼容清单/GitHub渠道一致，无用户配置或私钥。APK证书SHA256 `3c0a98ec3c9f37318525f5d0e4afb3417812215d625e48a9013b5ee649acb2b1`；中央清单 `974400d14d15b89af96d6a8826a9e39f3b42a69d1482541532e46e4897ca3158`。

实际云EXE独立home及仅System32 PATH通过内嵌运行时与DPAPI自检。包内官方Windows x64 **26.915.4065.0**只读读取35项列表，21/21接口及25/25功能依赖匹配。专用测试任务冷加载经包内生产HTTP端点确认，未向用户任务发送消息或已读指令。包内Node22.19.0的真实HTTP双转发续传6组和SQLite实际额度保存、新进程及官方断开后1/7/30天读回通过。

0.10.43→0.10.44双云包源码的隔离设备保存、强制结束测试子进程后保留、显式删除通过；现有6项设备/接入/更新/通知设置及通知草稿字节保持。原8份未完成Chat独立保全。

历史核心行为验证版本26.901.6511.0/26.903.8094.0（Windows x64）保持，其他版本按当前接口逐功能启用；本次新版本的接口匹配和任务激活不等于全部功能行为验证。Chat列表/历史及受能力约束的文字续写保持，文字续写未在本轮真实任务重测；普通Chat新建/图片与未完成Work不在包中。未运行APK/模拟器、升级现有客户端、重启官方或复测断开RDP时的冷加载。

标准发布器将八项资源上传草稿、逐项读回验签/哈希后公开；匿名latest双清单、固定版本完整APK/EXE及说明全部一致。证据在会话work/release-044/work/。发布记录提交不再次构建，产物仍固定于上述源码SHA。

...
try:
    for name in ['RemoteCodex.exe','RemoteCodex.apk','latest.json','android-latest.json']:
...
print(json.dumps({'version':versions[0],'exeCopies':len(list(root.glob('*.exe'))),'apkCopies':len(list(root.glob('*.apk')))}))
'''.replace('DIRECTORY', repr(directory)).replace('VERSION', repr(version))
    subprocess.run(['ssh', '-o', 'BatchMode=yes', host, 'python3 -'], input=script, text=True, check=True)

...
export function verifyBuildManifest(envelope, file, publicKey) {
  if (file === 'RemoteCodex.exe' || file === 'LimitRemoteCodex.exe') return verifyManifest(envelope, publicKey, file);
  if (file !== 'RemoteCodex.apk' && file !== 'LimitRemoteCodex.apk') throw Error('Unexpected build artifact');
  ...
  if (meta.schema !== 1 || meta.platform !== 'android' || meta.file !== file ||
      meta.packageName !== (file === 'LimitRemoteCodex.apk' ? 'com.anso.limitremotecodex' : 'com.anso.remotecodex') || parts.length !== 3 ||

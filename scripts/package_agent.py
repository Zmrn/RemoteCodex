...
Click the model name for an anchored model dropdown; the adjacent effort label opens the supported-effort slider and Fast toggle. Permissions have a separate nonmodal menu and can be preselected before creating or loading a task. Outside click and Escape dismiss. Settings UI tests: node scripts/verify-settings-ui.mjs and node scripts/verify-features-ui.mjs.

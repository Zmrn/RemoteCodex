...
subprocess.run([str(compiler),'/nologo','/target:exe','/platform:x64','/codepage:65001','/reference:System.Windows.Forms.dll','/reference:System.Drawing.dll','/reference:System.Web.Extensions.dll','/out:'+str(exe),str(ROOT/'windows/DesktopNotifications.cs'),str(ROOT/'test/DesktopNotificationsTests.cs')],check=True)
try:
    result=subprocess.run([str(exe),f'http://127.0.0.1:{server.server_port}',str(folder)],capture_output=True,text=True,encoding='utf-8',timeout=40,creationflags=subprocess.CREATE_NO_WINDOW)

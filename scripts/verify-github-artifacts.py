...
for package in (ROOT/'work/portable-build/payload.zip', ROOT/'dist/RemoteCodex.apk',
                ROOT/'work/portable-build-limit/payload.zip', ROOT/'dist/LimitRemoteCodex.apk'):
    with zipfile.ZipFile(package) as archive:
        ...
        if package.suffix == '.zip':
            expected_edition = 'limit' if 'portable-build-limit' in package.parts else 'full'
            if json.loads(archive.read('src/edition.json'))['edition'] != expected_edition:
                raise RuntimeError('Windows edition marker mismatch')
        else:
            expected_package = 'com.anso.limitremotecodex' if package.name.startswith('Limit') else 'com.anso.remotecodex'
            if json.loads((ROOT/'dist'/(package.stem+'.apk.build.json')).read_text())['packageName'] != expected_package:
                raise RuntimeError('Android package identity mismatch')

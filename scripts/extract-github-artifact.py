...
allowed = {'RemoteCodex.exe', 'RemoteCodex.apk', 'RemoteCodex.build.json', 'RemoteCodex.apk.build.json',
           'latest.json', 'android-latest.json', 'LimitRemoteCodex.exe', 'LimitRemoteCodex.apk',
           'LimitRemoteCodex.build.json', 'LimitRemoteCodex.apk.build.json', 'limit-latest.json',
           'limit-android-latest.json', 'RELEASE-NOTES.md', 'GITHUB-BUILD.json'}
with zipfile.ZipFile(archive) as z:
    entries = z.infolist()
    if len(entries) != len(allowed) or {e.filename for e in entries} != allowed:
        raise RuntimeError('Unexpected CI artifact entries')
    if sum(e.file_size for e in entries) > 300*1024*1024 or any(e.file_size > 100*1024*1024 for e in entries):
...
print('Extracted the four application packages and signed metadata; signature verification follows.')

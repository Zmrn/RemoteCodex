...
Codex 的消息、项目、任务、队列、问答、模型和权限界面共用 Windows 前端，均转交所选电脑。图片从系统文件选择器选取；点击图片可放大。附件下载读取电脑上的原文件，再由 Android 系统选择保存位置。首条消息仍需要文字；不同输入法能否将剪贴板图片作为文件交给 WebView 尚未实测，图片选择器是当前手机上传入口。

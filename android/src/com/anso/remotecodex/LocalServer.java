...
public final class LocalServer {
  ...
  private static byte[] read(InputStream in,int size)throws Exception{if(size<0||size>8*1024*1024)throw new Exception("请求过大");byte[] b=new byte[size];int n=0,k;while(n<size&&(k=in.read(b,n,size-n))>0)n+=k;if(n!=size)throw new EOFException();return b;}

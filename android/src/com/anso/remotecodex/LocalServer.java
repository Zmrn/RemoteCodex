...
public final class LocalServer {
  ...
  public static boolean allowed(String method,String route){String p=route.split("\\?",2)[0];if(method.equals("GET")&&p.matches("/api/(status|events|projects|threads|models|usage|updates|task-summary|compatibility/report)"))return true;if(method.equals("POST")&&p.matches("/api/(connect|threads|updates/(check|install|settings))"))return true;return method.equals("GET")?p.matches("/api/threads/[a-f0-9-]{36}(/(files|file|queue|media))?"):method.equals("POST")&&p.matches("/api/threads/[a-f0-9-]{36}/(follow|open|messages|interrupt|settings|queue|questions|approvals|read-receipt)");}

...
public final class MainActivity extends Activity {
  ...
  public synchronized void downloadImage(byte[] bytes,String filename)throws Exception{
    String mime=ReceivedImage.validate(bytes);
    ...
    io.execute(()->{try{
      ...
      runOnUiThread(()->startActivityForResult(new Intent(Intent.ACTION_CREATE_DOCUMENT).addCategory(Intent.CATEGORY_OPENABLE).setType(mime).putExtra(Intent.EXTRA_TITLE,name),31));
    }catch(Exception e){target.delete();pendingFile=null;message(e.getMessage());}});
  }

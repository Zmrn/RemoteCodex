...
public final class MainActivity extends Activity {
  ...
  @Override public void onCreate(Bundle state){super.onCreate(state);app=(App)getApplication();app.activity=new java.lang.ref.WeakReference<>(this);
    try{server=app.server();web=new WebView(this);web.setBackgroundColor(Color.rgb(16,23,34));setContentView(web);getWindow().setStatusBarColor(Color.rgb(16,23,34));getWindow().setNavigationBarColor(Color.rgb(16,23,34));
      ...
      web.setWebViewClient(new WebViewClient(){@Override public boolean shouldOverrideUrlLoading(WebView view,WebResourceRequest r){String u=r.getUrl().toString();if(u.startsWith(server.origin+"/"))return false;if(r.isForMainFrame()&&("https".equals(r.getUrl().getScheme())||"http".equals(r.getUrl().getScheme())))try{startActivity(new Intent(Intent.ACTION_VIEW,r.getUrl()));}catch(Exception e){message("无法打开外部链接");}return true;}
        @Override public void onPageStarted(WebView v,String u,android.graphics.Bitmap icon){if(!u.startsWith(server.origin+"/"))v.stopLoading();}});
      web.setWebChromeClient(new WebChromeClient(){@Override public boolean onShowFileChooser(WebView v,ValueCallback<Uri[]> callback,FileChooserParams params){if(chooser!=null)chooser.onReceiveValue(null);chooser=callback;try{Intent pick=new Intent(Intent.ACTION_OPEN_DOCUMENT).setType("image/*").addCategory(Intent.CATEGORY_OPENABLE).putExtra(Intent.EXTRA_MIME_TYPES,new String[]{"image/png","image/jpeg","image/webp"});startActivityForResult(pick,30);}catch(Exception e){chooser.onReceiveValue(null);chooser=null;}return true;}});
  ...
  @Override protected void onResume(){super.onResume();if(app!=null){app.activity=new java.lang.ref.WeakReference<>(this);app.updates.foreground();}if(web!=null)web.onResume();if(pendingInstall&&getPackageManager().canRequestPackageInstalls()){pendingInstall=false;installUpdate();}}
  @Override protected void onPause(){if(web!=null){web.evaluateJavascript("window.remoteCodexSaveDrafts?.()",null);web.onPause();}super.onPause();}
  @Override protected void onDestroy(){if(web!=null){web.stopLoading();web.destroy();}if(chooser!=null)chooser.onReceiveValue(null);if(app!=null&&app.activity.get()==this)app.activity.clear();super.onDestroy();}
  ...
  @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);if(request==30&&chooser!=null){chooser.onReceiveValue(WebChromeClient.FileChooserParams.parseResult(result,data));chooser=null;}if(request==31&&pendingFile!=null){File source=pendingFile;pendingFile=null;if(result==RESULT_OK&&data!=null&&data.getData()!=null){Uri uri=data.getData();io.execute(()->{try(InputStream in=new FileInputStream(source);OutputStream out=getContentResolver().openOutputStream(uri)){byte[] b=new byte[65536];int n;while((n=in.read(b))!=-1)out.write(b,0,n);message("已保存原文件");}catch(Exception e){message("保存失败，请重试");}finally{source.delete();}});}else source.delete();}}
}

...
public final class Updates {
  ...
  private HttpURLConnection open(String file)throws Exception{URL u=new URL(base()+file);if(!Arrays.asList("http","https").contains(u.getProtocol())||u.getUserInfo()!=null)throw new Exception("更新地址无效");Devices.resolve(u.getHost());HttpURLConnection c=(HttpURLConnection)u.openConnection(Proxy.NO_PROXY);c.setInstanceFollowRedirects(false);c.setConnectTimeout(10000);c.setReadTimeout(30000);if(c.getResponseCode()!=200){c.disconnect();throw new Exception("更新资源暂时不可用");}return c;}
  ...
  private void download()throws Exception{
    ...
    if(!cached){phase="downloading";progress=0;File tmp=new File(ready().getParentFile(),"download.tmp");HttpURLConnection c=open("RemoteCodex.apk");long total=0,limit=latest.getLong("bytes");try(InputStream in=c.getInputStream();OutputStream out=new FileOutputStream(tmp)){byte[] b=new byte[65536];int n;while((n=in.read(b))!=-1){total+=n;if(total>limit)throw new Exception("APK 超出签名清单大小");out.write(b,0,n);progress=(int)(100*total/limit);}}finally{c.disconnect();}validateApk(tmp);if(!tmp.renameTo(ready()))throw new Exception("无法保存更新包");}

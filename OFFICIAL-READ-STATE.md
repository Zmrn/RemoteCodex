...
## 2026-09-12：超长历史与较早分页中的最新回复

旧版分段历史固定reportReceipt=null；通知核验又直接读整轮read_thread，即使正文可读，仍可能因官方帧上限而无法同步。部分legacy owner轮次也没有进入正文合并。此次分别修正：

- 分段历史只显示内容；必须匹配当前owner最新已完成回复的任务、轮次、消息ID和全文，才从该owner生成回执。历史中的completed不会单独授权已读。最新回复在较早传输页时也可核验，并只在当前owner返回相同token期间保留本次可见凭据，头部刷新不丢弃；新回复或owner未知立即失效。
- 通知前读取官方实时任务元数据并重新follow，等待新owner状态；核对同连接/身份/owner、idle和最新回报token后，再发送既有readStateChanged v3。不再要求序列化整个超大历史回合；元数据路径不能使用SQLite/rollout授权。未知发送不重放，仍读回官方分区确认。
- canonical与legacy owner轮次均可覆盖历史中的同轮内容，包括官方删除后的空数组。只缺少items数组时才沿用历史显示。

800ms真实可见、前台焦点和侧栏遮挡检查继续生效；只看旧内容、不完整回复或缺少最新实时证据时不清点。客户端回执和当前token都是短时请求凭据，不存储官方未读状态。蓝点只有新官方观察为已读时才消失，刷新中未知不会当作成功。

新增test/thread-sync-read.test.mjs验证索引安全边界、分段内容精确匹配、元数据+新快照及超限正文不参与通知。共享UI以最新回复位于较早页且后有65条命令记录复现，验证头部刷新、正式已读观察和任务卡跳转；原已读UI仍覆盖失败重试、断线、失焦、草稿与未知发送。实际问题任务只读，复查时官方已经是已读，未发送真实通知；本轮不打包、不执行APK。

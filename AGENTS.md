...
- 2026-09-22：修复图片文件同路径覆盖后显示旧缓存。目标端用文件身份/大小/纳秒mtime/ctime等生成contentKey，每次读取同路径只stat一次、不预读大图；保留原任务媒体授权ID。前端下载缓存纳入contentKey，与已有DOM/断点版本隔离一致，文件变更重载、未变不闪烁、迟到旧下载不污染新图。357 Node22.19.0、新覆盖双尺寸8组、原稳定6组/进度续传10组/手势4组/GIF2组/Chat与隔离设备强制重启保护通过；旧前端可复现旧字节，修复后新消息/放大/下载均为新版。新专项纳Checks，详见IMAGE-PREVIEW-VALIDATION.md。版本仍0.10.45，未打包发布安装/执行APK/重启官方或读写用户任务，原8份Chat独立保全。后续打包需Windows接入端和控制端一起更新，第三方HTTPS与未完成Chat/Work不扩展。

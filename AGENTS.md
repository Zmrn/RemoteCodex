...
- 2026-09-11 大历史修复：官方read_thread缩至一轮仍失败时，内容页可从当前官方home只读分段加载原始rollout，图片按需获取；只用实时官方列表中的本机Codex任务、核对文件/任务/轮次身份，游标和图片绑定当前文件版本，缓存页返回前也重新核对。索引只存内存偏移/ID，不维护消息、已读或运行状态副本；原写入和已读核验不得降为磁盘证据。文件变化会丢弃过期显示，失败保留正文/草稿。真实问题任务官方26.903.9818.0只读遍历32页820项及独立PNG加载通过；267项Node22、新共享UI6组和相关旧回归通过。未完成Chat独立保留，版本仍0.10.32；未构建/发布/安装、执行APK或写真实任务。限制与复测见HISTORY-READ.md，关键入口test/rollout-history.test.mjs、scripts/verify-rollout-history-ui.mjs；后者已纳Checks。
